...
export function PlatformCard({
  ...
  return (
    <motion.div
      ...
      className={cn(
        "flex flex-col h-full bg-white dark:bg-[#1c2128] rounded-2xl border border-gray-200 dark:border-white/10 p-8 transition-all duration-300 relative overflow-hidden group shadow-sm dark:shadow-none",
        colors[gradientColor]
      )}
    >
      {/* Badge */}
      <div className="mb-6">
        <span className="px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider border border-gray-300 dark:border-white/20 bg-gray-100 dark:bg-white/5 text-gradient">
          {badge}
        </span>
      </div>
      ...
      <div className="flex items-center gap-4 mb-4">
        <div className={cn("p-3 rounded-lg bg-gray-100 dark:bg-white/5", iconColors[gradientColor])}>
      ...
      {/* Gradient Glow */}
      <div className={cn(
        "absolute -bottom-20 -right-20 w-64 h-64 rounded-full blur-[100px] opacity-10 dark:opacity-20 pointer-events-none transition-opacity group-hover:opacity-20 dark:group-hover:opacity-30",
